...
Febrero$SoloSeguro<-as.factor(Febrero$SoloSeguro)
...
#TrainD2 <- createDataPartition(ClientesInteres$IdCuentaTarjeta, p=0.7, list=FALSE)

trainingD <- ClientesInteres[ TrainD2, ]
testingD <- ClientesInteres[ -TrainD2, ]
...
Buenos<-sample_n(subset(trainingD,Objetivo==0), 72168,replace=FALSE)
...
tree2 <- rpart(ObjetivoCharacter ~ 
                 ...
                 DifCupoUtilizado+
                 EstadoCivil
               ,data = BalanceadaD, control = rpart.control(cp = 0.0015))
...
Codificada["CodCupoUtiAvance"]<-"Mayor a 202"
Codificada[which(Codificada$CupoUtilizadoAvance<202),]$CodCupoUtiAvance<-"Menor a 202"
...
Codificada["NormDifCupoUtilizado"]<-as.numeric(scale(Codificada$DifCupoUtilizado))

################## Cupo Utilizado en Avances Normalizado###############3
Codificada["NormCupoUtilizadoAvance"]<-as.numeric(scale(Codificada$CupoUtilizadoAvance))

################## Cupo Utilizado Normalizado###############3
Codificada["NormCupoUtilizado"]<-as.numeric(scale(Codificada$CupoUtilizado))
...
table(Codificada$Decil,Codificada$ObjetivoCharacter)
