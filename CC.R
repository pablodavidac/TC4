...
####################################################################################
########################Regresion#######################################
Activo<-subset(DiciembreI,TipoCliente=="Activo")
Inactivo<-subset(DiciembreI,TipoCliente=="Inactivo")

Diciembre<-Inactivo
Diciembre<-subset(Diciembre,`Segmento_TDC Nuevo`!="4. MovilizaciÛn B" &
              `Segmento_TDC Nuevo`!="5. NormalizaciÛn A" &
              `Segmento_TDC Nuevo`!="6. NormalizaciÛn B" &
              `Segmento_TDC Nuevo`!="7. Inactivos" &
              `Segmento_TDC Nuevo`!="8. No Asignado")
#####################Conjunto de Entrenamiento###############################
TrainD2 <- createDataPartition(Diciembre$IdCuentaTarjeta, p=0.7, list=FALSE)
trainingD <- Diciembre[ TrainD2, ]
testingD <- Diciembre[ -TrainD2, ]
...
muestra<-sample_n(subset(trainingD, Objetivo==0), 1427,replace = FALSE)  #Segmentos de interes
...
##################Inactivos#################
Diciembre["Afinidad"]<-"NA"
Diciembre[which(Diciembre$AfinidadTarjeta=="CUOTA FACIL"),]$Afinidad<-0
Diciembre[which(Diciembre$AfinidadTarjeta=="TARJETA ALIA - CUOTA FACIL"),]$Afinidad<-1
Diciembre[which(Diciembre$AfinidadTarjeta=="TARJETA ALIA - GOLD"),]$Afinidad<-1

Diciembre[which(Diciembre$AfinidadTarjeta=="TARJETA ALIA - BLACK"),]$Afinidad<-1
Diciembre[which(Diciembre$AfinidadTarjeta=="TARJETA ALIA - PLATINUM"),]$Afinidad<-1
Diciembre[which(Diciembre$Afinidad=="NA"),]$Afinidad<-0
###########################################
...
fitted.results <- predict(modA2,newdata=testingD,type='response')
...
View(table(Diciembre$Decil,Diciembre$Mesdesercion))
...
aux <- subset(Diciembre,select=c("IdCuentaTarjeta","Objetivo",
            ...
            "Score",
            "Decil"))
write.csv(aux,file = "ClientesActivos2.csv")
...
Aux<-subset(aux, SoloSeguro=="SI" | Menos8Autorizaciones6M==1)

####################Bajar Segmento de TDC########################
...
SB_SegemtoDic2015 <- sqlExecute(odbcChannel, "SELECT * FROM SB_SegemtoDic2015", fetch = TRUE)

odbcClose(odbcChannel)

rm(odbcChannel)

SB_SegemtoDic2015<-subset(SB_SegemtoDic2015,select=c("IdCuentaTarjeta","Segmento_TDC"))
colnames(SB_SegemtoDic2015)[2]<-"Segmento_TDC Nuevo"   

Diciembre<-left_join(Diciembre,SB_SegemtoDic2015,by="IdCuentaTarjeta")
